	...
	if event is InputEventMouseButton:
		...
		if clicked == 0:
			...
			if event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
				clicked += 1
				choose_attack_defense(mesh.scale == Vector3(1.2, 1.2, 1.2))
		else:
			clicked = 0
...
func choose_attack_defense(active):
	# Remove attack from the list and de-emphasize the icon
	if active:
		mesh.scale = Vector3(1, 1, 1)
		player.choose_attack_defense("defense", self.name, false)
	# Add attack to the list and emphasize the icon
	else:
		mesh.scale = Vector3(1.2, 1.2, 1.2)
		player.choose_attack_defense("defense", self.name, true)
